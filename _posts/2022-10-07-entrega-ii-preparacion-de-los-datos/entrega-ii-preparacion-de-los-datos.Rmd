...
Después de analizar las características de las distintas variables, se comprobó que dos variables que deberían de ser numéricas estaban como caracteres, por lo que se realizaron los cambios necesarios en el tipo de variables transformándose en numéricas.
...
boxplot(data$edad, col="lightblue", main="Edad Boxplot",
        ylab="Edad")
stats_edad = boxplot.stats(data$edad)
...
library(VIM)
dataKNN<-kNN(data, variable=c("J0","J45","cil","eje","orient_astigm"), k=5)
summary(dataKNN)
...
ggplot(aes(x=dataKNN$CA, y=dataKNN$M)) +
...
ggplot(aes(x=dataKNN$CA, y=dataKNN$RC)) +
  geom_point(color="lightgreen", size=2, shape=23) + 
  labs(x = "Longitud Axial del Ojo", y = "Radio de Curvatura del Ojo",
       title = "Relacion entre la longitud axial y el radio de curavatura del ojo")

ggplot(aes(x=dataKNN$CA, y=dataKNN$edad)) +
  geom_point(color="violet", size=2, shape=23) + 
  labs(x = "Longitud Axial del Ojo", y = "Edad las personas",
