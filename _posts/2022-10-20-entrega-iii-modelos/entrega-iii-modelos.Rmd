...
```{r}
model_reg_mult <- lm( M ~ CA + RC, data = dataKNN)
class(model_reg_mult)
summary(model_reg_mult)
library(texreg)
screenreg(model_reg_mult)
```

```{r}
model_reg_simp <- lm( CA ~ edad, data = dataKNN)
class(model_reg_simp)
summary(model_reg_simp)
screenreg(model_reg_simp)
```
